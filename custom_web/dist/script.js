...
function onResults(results) {
    ...
    if (results.multiHandLandmarks && results.multiHandedness) {
        for (let index = 0; index < results.multiHandLandmarks.length; index++) {
            //const hand_position = results.multiHandLandmarks;
            const classification = results.multiHandedness[index];
            const isRightHand = classification.label === 'Right';
            const landmarks = results.multiHandLandmarks[index];
            
            check=true
            // var xmin,xmax,ymin,ymax,bowH,bowH    
            var xmin = Math.min(...landmarks.map(o => o.x*w))

            var ymin = Math.min(...landmarks.map(o => o.y*h))

            var xmax = Math.max(...landmarks.map(o => o.x*w))
            ...
            canvasCtx.strokeRect(xmin-30,ymin-30, xmax-xmin+40, ymax-ymin+40);
